...
class NotificationService:
    ...
    async def notify_new_book(self, book: Any, owner_display: str):
        """
        Уведомление о новой книге всем пользователям (через group chat).

        FIX #26: параметр переименован в owner_display.
        Шаблон добавляет @ только если это username (без @).
        Вызывающий код (book_service) передаёт голый username или full_name.
        """
        # Добавляем @ только если owner_display выглядит как username (без пробелов)
        if " " not in owner_display:
            mention = f"@{owner_display}"
        else:
            mention = owner_display
        ...
        payload = {
            ...
            "message":  msg,
            "book_id":  book.id,
            "meta":     {"owner_display": owner_display},
        }
    ...
    async def notify_waitlist_available(self, book: BookTable, user_id: int):
        """Уведомление первому в очереди о доступности книги (ТЗ 3.2.2, 4.3)."""
        msg = (
            f"🔥 Книга <b>«{book.title}»</b>, которую вы ждали, теперь свободна!\n"
    ...
    async def notify_reservation_approved(self, book: BookTable):
        ...
        Вызывается с объектом book, ПЕРЕЧИТАННЫМ из БД после update_status —
        иначе return_due_date будет None.
        """
        if book.return_due_date is None:
            print(f"⚠️ notify_reservation_approved: book #{book.id} has no return_due_date")
            date_str = "не указана"
        else:
            date_str = book.return_due_date.strftime("%d.%m.%Y")

        msg = (
            f"✅ Ваша заявка на книгу <b>«{book.title}»</b> одобрена!\n"
    ...
    async def notify_about_overdue(self, book: BookTable, borrower_username: str = None):
        """Уведомление о просроченной книге заёмщику и владельцу (ТЗ 4.2)."""
...
class OverdueChecker:
    ...
    async def check_overdue(self):
        ...
        async with self.SessionLocal() as session:
            ...
            # -------------------------------------------------------------------
            # 1. Помечаем просроченные книги как OVERDUE и уведомляем
            # -------------------------------------------------------------------
            ...
            for book in overdue_books:
                # FIX #8: добавлено условие status == BORROWED в UPDATE.
                # Без него возможна гонка: книгу вернули между SELECT и UPDATE,
                # статус сменился на AVAILABLE, но UPDATE перетирает его в OVERDUE.
                # Теперь UPDATE выполняется только если статус всё ещё BORROWED.
                updated = await session.execute(
                    sa_update(BookTable)
                    .where(
                        BookTable.id     == book.id,
                        BookTable.status == BookStatus.BORROWED,   # ← атомарная проверка
                    )
                    .values(status=BookStatus.OVERDUE)
                    .returning(BookTable.id)
                )
                if updated.scalar() is None:
                    # Книгу успели вернуть или сменить статус — пропускаем
                    print(f"⚙️ Book #{book.id}: status changed before OVERDUE update, skipping")
                    continue
            ...
            # -------------------------------------------------------------------
            # 2. Напоминания за 3 и за 1 день до срока
            # -------------------------------------------------------------------
            # Проверяем узкое окно ±30 минут вокруг порогов, чтобы не спамить
            # каждый час. За цикл в 1 час окно гарантированно попадает ровно раз.
...
async def run_background_tasks():
    ...
    print(f"🚀 Background tasks started (Push Mode → {BOT_WEBHOOK_URL})...")
