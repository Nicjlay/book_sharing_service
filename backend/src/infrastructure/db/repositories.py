...
class UserRepository:
    ...
    async def get_or_create_user(
        ...
        FIX #2, #20: параметр is_admin удалён.
        - При создании нового пользователя is_admin = False (безопасное умолчание).
        - Для существующих пользователей is_admin НЕ перезаписывается:
          права должны управляться только через БД / admin-эндпоинт,
          чтобы обычный login не сбрасывал и не повышал права.
        """
        ...
        if not user:
            user = UserTable(
                ...
                full_name=full_name,
                username=username,
                is_admin=False,   # новые пользователи всегда не-admin
            )
            self.session.add(user)
        else:
            # Обновляем только публичные поля профиля
    ...
    async def set_admin(self, tg_id: int, is_admin: bool) -> Optional[UserTable]:
        ...
        Должен вызываться только из защищённого admin-эндпоинта.
        """
        stmt = (
            update(UserTable)
        ...
        if result.scalar() is None:
            return None
        return await self.session.get(UserTable, tg_id)
...
class BookRepository:
    ...
    async def try_reserve(self, book_id: int, user_id: int) -> bool:
        """
        Атомарно переводит книгу AVAILABLE → RESERVED для конкретного пользователя.
        UPDATE ... WHERE status='available' RETURNING гарантирует, что только один
        из конкурентных запросов победит (optimistic locking без явной блокировки).
        """
        stmt = (
            update(BookTable)
            .where(
    ...
    async def get_books_lightweight(self, limit: int = 2000) -> List[dict]:
        """
        FIX #15: Облегчённый запрос — только id/title/author без JOIN-ов owner/borrower.
        Используется как источник кандидатов для Python fuzzy-поиска.
        Экономит 3 SQL-запроса и ~N*2 записей пользователей в памяти.
        """
        stmt = (
        ...
        result = await self.session.execute(stmt)
        return [{"id": r.id, "title": r.title, "author": r.author} for r in result]
    ...
    async def pop_first_waiter(self, book_id: int) -> Optional[int]:
        """
        FIX #9: Атомарно извлекает ПЕРВОГО по времени постановки в очередь пользователя.

        Вместо уведомления всех сразу (старый pop_waitlist) уведомляем только первого.
        Остальные остаются в очереди и получат уведомление при следующем возврате.
        Это исключает ситуацию, когда N человек «гонятся» за одной книгой,
        все они удалены из очереди, но победить может только один.
        """
        subquery = (
